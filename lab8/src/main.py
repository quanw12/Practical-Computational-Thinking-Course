...
def simple_translate(text, src_lang='en', dest_lang='vi'):
    """
    Simple translation using Google Translate API (compatible with all Python versions)
    Based on the tutorial guidance for reliable translation
    """
    try:
        # Using Google Translate web interface (educational purposes)
        ...
        headers = {
            'User-Agent': 'VietnamDiscoveryExplorer/1.0 (educational-translation-service)'
        }
        ...
        if result and len(result) > 0 and len(result[0]) > 0:
            ...
            return translated_text, src_lang, dest_lang
        else:
        ...
        raise ValueError(f"Translation parsing error: {str(e)}")
...
def translate_text():
    ...
    try:
        ...
        # Perform translation using our simple method
        try:
            translated_text, src_detected, dest_lang = simple_translate(text, source_lang, target_lang)
            
            return jsonify({
                'success': True,
                'original_text': text,
                'translated_text': translated_text,
                'source_language': src_detected,
                'target_language': dest_lang,
                'message': f'Successfully translated from {src_detected} to {dest_lang}'
            })
            
        except ValueError as translate_error:
            return jsonify({
                'success': False,
                'message': f'Translation service error: {str(translate_error)}'
            }), 500
...
def home():
    ...
    return jsonify({
        ...
        'endpoints': {
            ...
            'translate': '/api/translate (POST) - Translate English text to Vietnamese',
            'auth/verify': '/api/auth/verify (POST) - Verify Firebase authentication token',
            'user/profile': '/api/user/profile (GET) - Get user profile information',
